...
SELECT name FROM people WHERE name = 'Crusty the Clown';
...
SELECT * FROM people WHERE name = 'Eric Cartman';
DELETE FROM people WHERE name = 'Eric Cartman';
SELECT * FROM people WHERE name = 'Eric Cartman';

-- 8.  The cinema has just heard that they will be holding an exclusive midnight showing of 'Avengers: Infinity War'!! Create a new entry in the 'movies' table to reflect this.

INSERT INTO movies (title, show_time) VALUES ('Avengers: Infinity War', '0.00');
SELECT * FROM movies;
...
-- DELETE FROM people WHERE name IN ([name], [name], [name])
